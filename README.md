...
| 推送 / 拉取 | 自动处理无上游分支的情况（`-u origin <branch>`）；**没有远程仓库时，配置了 GitHub Token 会自动创建私有仓库再推送** |
...
前往 [Releases](https://github.com/Bavoch/hello-gitty/releases) 下载对应平台的安装包（目前提供 macOS `.app` / `.dmg`）。

### 从源码构建

需要 [Node.js](https://nodejs.org/) 18+ 和 [Rust](https://www.rust-lang.org/) 环境。
